...
<html>
   ...
   <?php
        ...
        $_sql        = "SELECT pegawai.nik,
                       ...
                       and pegawai.departemen=departemen.dep_id where pegawai.stts_aktif<>'KELUAR' and pegawai.bpd='Y' order by pegawai.id ASC  ";
        ...
        if(mysql_num_rows($hasil6)!=0) {
            ...
            while($data = mysql_fetch_array($hasil6)) {   
                ...
                if(($status=="FT")||($status=="T")||($status=="CPT")||($status=="CK")){
                    ...
                    $_sqlgp    = "SELECT `gapok1`, `gapok2`, `gapok3`
                                from pendidikan  where tingkat like '%".substr($pendidikan,0,7)."%' order by gapok1 desc limit 1 ";
                    $hasilgp   = bukaquery($_sqlgp);
                    $barisgp   = mysql_fetch_row($hasilgp);
                    $gapokgp     = 0;
                    if($masker<1){
