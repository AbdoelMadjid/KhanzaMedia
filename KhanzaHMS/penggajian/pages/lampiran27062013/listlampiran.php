	...
	$hasiltambahan=bukaquery($_sql);
		...
		//untuk mencari nilai referensinya
                if ($action!="CARI") {
                    ...
                    hapusinput("delete from  tambahanpotongan");
                }
		while($barisindex = mysql_fetch_array($hasilindex)) {		
		    ...
		    $ttlindex=$barisindex[2]+$indexjaga;
                ...
                while($baristambahan = mysql_fetch_array($hasiltambahan)) {
                    if($baristambahan["pengurang"]>0){
                        $masa_kerja=0;
                          if($baristambahan[9]<1){
                             $masa_kerja=0;
                          }else if(($baristambahan[9]>=1)&&($baristambahan[9]<2)){
                             $masa_kerja=2;
                          }else if(($baristambahan[9]>=2)&&($baristambahan[9]<3)){
                             $masa_kerja=4;
                          }else if(($baristambahan[9]>=3)&&($baristambahan[9]<4)){
                             $masa_kerja=6;
                          }else if(($baristambahan[9]>=4)&&($baristambahan[9]<5)){
                             $masa_kerja=8;
                          }else if(($baristambahan[9]>=5)&&($baristambahan[9]<6)){
                             $masa_kerja=10;
                          }else if(($baristambahan[9]>=6)&&($baristambahan[9]<7)){
                             $masa_kerja=12;
                          }else if($baristambahan[9]>=7){
                             $masa_kerja=14;
                          }
                          
                          $total=($baristambahan[8]+$masa_kerja+$baristambahan[10]+$baristambahan[11]);   
                          
                          $_sql2         = "SELECT normal-$jumlahlibur,jmlhr FROM set_tahun";
			  $hasil2        = bukaquery($_sql2);
			  $baris2        = mysql_fetch_row($hasil2);
			  $jmlmsk         = $baris2[0];
			  if($baristambahan[13]==-1){
			     $jmlmsk=0;
			  }else if($baristambahan[13]==-2){
			     $jmlmsk=$baris2[1]-4;
			  }else if($baristambahan[13]==-3){
			     $jmlmsk=$baris2[1]-2-$jumlahlibur;
			  }else if($baristambahan[13]!=0){
			     $jmlmsk=$baristambahan[13];
			  }else if(!($baristambahan[13]==0)){
			     $jmlmsk=$baris2[0];
			  }

                            $_sql3    = "SELECT sum(jml)
                            from jgmlm  where id='$baristambahan[0]'
                            and tgl like '%".$tahun."-".$bulan."%' group by id";
                            $hasil3   = bukaquery($_sql3);
                            $baris3   = mysql_fetch_row($hasil3);
                            $jgmlm    = $baris3[0];
                            ...
                            $_sql4    = "SELECT sum(jml)
                            from ketidakhadiran  where id='$baristambahan[0]'
                            and tgl like '%".$tahun."-".$bulan."%' and jns='A' group by id";
                            $hasil4   = bukaquery($_sql4);
                            $baris4   = mysql_fetch_row($hasil4);
                            $ttla     = $baris4[0];
                            ...
                            $_sql8    = "SELECT sum(jml)
                            from tambahjaga  where id='$baristambahan[0]'
                            and tgl like '%".$tahun."-".$bulan."%' group by id";
                            $hasil8   = bukaquery($_sql8);
                            $baris8   = mysql_fetch_row($hasil8);
                            $ttltmb   = $baris8[0];
                            ...
                            $_sql22="";
                            if($baris[6]!="DIR"){
                                $_sql22  ="SELECT ($ttln/sum(n))*100 from indekref where kdindex='$baristambahan[6]'";
                            }else if($baris[6]=="DIR"){
			    ...
			    $ttlindex=$total+$indexjaga+$indexpas;                                                
                         ...
                         $_sqltambahan="SELECT ($ttlindex/sum(indextotal.ttl))*((indexins.persen/100)*total_insentif) 
                                            * ($baristambahan[12]/100)
                                            ...
                                            indextotal.kdindex='$baristambahan[6]'"; 
                         $hasilpotongan=bukaquery($_sqltambahan);                                           
                                            $barispotongan= mysql_fetch_array($hasilpotongan);
                                            $ttlpotongan=$barispotongan[0];
                         if ($action!="CARI") {
                            bukainput("insert into tambahanpotongan  values('$baristambahan[6]','$ttlpotongan')");                             
                         }
                    }    
                }
