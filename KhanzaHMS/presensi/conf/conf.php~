...
//function
    ...
    function reportsqlinjection(){
     ...
     function Tambah2($tabelname,$attrib) {
	...
	function loadJam(){
                $jamini=date('H');
		echo "<option selected>$jamini</option>";
	...
	function loadMenit(){
                $menitini=date('i');
		echo "<option selected>$menitini</option>";
        ...
        function loadDetik(){
                $detikini=date('s');
		echo "<option selected>$detikini</option>";
		for($menit=0; $menit<=60; $menit++){
			$menit_leng=strlen($menit);
			if ($menit_leng==1)
			$i="0".$menit;
